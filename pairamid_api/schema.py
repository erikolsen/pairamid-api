...
class TeamSchema(CamelCaseSchema):
    ...
    def last_active(self, obj):
        pairing_session = (
            obj.pairing_sessions.order_by(desc(PairingSession.created_at))
...
class FeedbackUserSchema(CamelCaseSchema):
    class Meta:
        model = User
        fields = ["full_name"]
...
class FeedbackSchema(CamelCaseSchema):
    ...
    class Meta:
        model = PairingSession
        include_relationships = True
        fields = ("id", "author_name", "message", "created_at", "tags", "recipient")

    recipient = fields.Nested(FeedbackUserSchema)
    tags = fields.Nested(FeedbackTagSchema, many=True)
...
class FullUserSchema(UserSchema):
    ...
    feedback_sent = fields.Nested(FeedbackSchema, many=True)

...
def user_show(id):
    team_member = flask_praetorian.current_user()
    if str(team_member.uuid) == id:
...
def user_login():
    ...
    try:
        team_member = guard.authenticate(email, password)
        result = {
            "access_token": guard.encode_jwt_token(team_member),
